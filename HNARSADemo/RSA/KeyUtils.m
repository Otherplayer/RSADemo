...
#ifndef BeeExceptions_h
#define BeeExceptions_h

#define BeeException                        @"BeeException"
#define BeeSecurityException                @"BeeSecurityException"
#define BeeChannelRuntimeException          @"BeeChannelRuntimeException"
#define BeeBootstrapRuntimeException        @"BeeBootstrapRuntimeException"
#define BeeInvalidChannelException          @"BeeInvalidChannelException"
#define BeeIllegalStateException            @"BeeIllegalStateException"
#define BeeInterruptedException             @"BeeInterruptedException"
#define BeeTransportException               @"BeeTransportException"
#define BeeIOException                      @"BeeIOException"
#define BeeUnavailableTopic                 @"BeeUnavailableTopic"
#define BeeBeeInvalidConfigurationException @"BeeBeeInvalidConfigurationException"
#define BeeTcpProtocolException             @"BeeTcpProtocolException"
#define BeeKeyPairGenerationException       @"BeeKeyPairGenerationException"
#define BeeUnableOpenFile                   @"BeeUnableOpenFile"
#define BeeRuntimeException                 @"BeeRuntimeException"

#endif /* BeeExceptions_h */
...
//#import "BeeLogging.h"
//#import "BeeExceptions.h"

#define TAG @"KeyUtil >>>"

#define KEY_PAIR_SIZE   2048

static const uint8_t publicKeyIdentifier[]  = "RSAUtil_PubKey";
static const uint8_t privateKeyIdentifier[] = "RSAUtil_PrivKey";
...
+ (KeyPair *)generateKeyPairWithPrivateTag:(NSData *)privateTag publicTag:(NSData *)publicTag {
    ...
    NSLog(@"%@ Removing key pair with same tags if exists", TAG);
    ...
    if (sanityCheck == noErr && publicKeyRef != NULL && privateKeyRef != NULL) {
        NSLog(@"%@ Successfully generated new key pair", TAG);
        return [[KeyPair alloc] initWithPrivateKeyRef:privateKeyRef publicKeyRef:publicKeyRef];
    } else {
        NSLog(@"%@ Failed to generate new key pair", TAG);
        [NSException raise:BeeKeyPairGenerationException format:@"Failed to generate new key pair!"];
        return nil;
    }
...
+ (SecKeyRef)getKeyRefByTag:(NSData *)tag {
    ...
    if (sanityCheck != noErr) {
        NSLog(@"%@ SecKeyRef with tag [%@] not found. OSStatus: %i", TAG, [tag hexadecimalString], (int)sanityCheck);
        keyReference = NULL;
    }
...
+ (NSData *)getPlainPublicKeyByTag:(NSData *)tag {
    ...
    if (sanityCheck != noErr) {
        NSLog(@"%@ Can't get public key bytes by tag. OSStatus: %i", TAG, (int)sanityCheck);
    }
...
+ (SecKeyRef)storePublicKey:(NSData *)publicKey withTag:(NSData *)tag {
    ...
    if (!processedKey) {
        NSLog(@"%@ Wasn't able to strip header for remote public key, passing plain public key to keychain", TAG);
        processedKey = publicKey;
    }
    NSLog(@"%@ Remote public key: %@", TAG, [publicKey hexadecimalString]);
    ...
    if(sanityCheck != noErr){
        NSLog(@"%@ Problem adding the remote public key to the keychain. OSStatus: %i", TAG, (int)sanityCheck);
        return NULL;
    }
...
+ (void)removeKeyByTag:(NSData *)tag {
    ...
    if (sanityCheck == noErr) {
        NSLog(@"%@ Successfully removed key", TAG);
    } else {
        NSLog(@"%@ Unable to remove key, status: %i", TAG, (int)sanityCheck);
    }
}

+ (void)deleteExistingKeyPair {
    NSLog(@"%@ Goint to remove default private key", TAG);
    [self removeKeyByTag:[self defaultPrivateKeyTag]];
    
    NSLog(@"%@ Goint to remove default public key", TAG);
    [self removeKeyByTag:[self defaultPublicKeyTag]];
}

+ (SecKeyRef)getKeyRefWithPersistentKeyRef:(CFTypeRef)persistentRef {
    if (persistentRef == NULL) {
        NSLog(@"%@ PersistentRef object cannot be NULL", TAG);
        return NULL;
    }
    ...
    if (sanityCheck != noErr) {
        NSLog(@"%@ Can't get key ref with persistent key ref, status: %i", TAG, (int)sanityCheck);
    }
